...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $buildScript)) {
        ...
        # PyInstaller writes a DEPRECATION warning to stderr. With
        # $ErrorActionPreference=Stop (set at top of script) that becomes a
        # terminating error and aborts the whole runner at the first warning.
        # Relax it for the native call, and do NOT pipe the output - a pipe
        # would set $LASTEXITCODE to the last cmdlet (Add-Content), not the
        # build. Capture output, save the real exit code, then log it.
        $prevEAP = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
        $buildOutput = & powershell -ExecutionPolicy Bypass -File $buildScript 2>&1
        $buildExit = $LASTEXITCODE
        $ErrorActionPreference = $prevEAP
        $buildOutput | Out-String | Add-Content -Path $LogFile -Encoding UTF8
        if ($buildExit -eq 0) {
            ...
            $Report.BuildResult = "FAIL (code $buildExit)"
