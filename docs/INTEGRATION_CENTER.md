...
AICC as the single control center for every project in the ecosystem. This
document is the design for **increment 1**: a project registry, read-only
health collectors, and a "Projects" UI surface. Design first, then code —
the implementation in `command_center/integration/` must not exceed what is
written here.

## Why

Five repositories are being developed in parallel — `aios` (the engine),
`ai-command-center` (this control plane), `ESF-Enterprise-Clean-Starter`
(domain module), `AML-Governance-Platform`, and `AICOS`. Today their state
lives in five terminal windows and five GitHub tabs. The Integration Center
gives the operator one page that answers, per project: *is the working tree
clean, is CI green, what PRs are open, when did it last move, and what AICC
tasks target it.*

## Data model — project registry

Store: **`data/integration_registry.json`** (+ `integration_registry.lock`).
A JSON dict store in the style of `project_config.json` — *not* a new
`runtime.db` table: the registry is operator configuration (machine-local
repo paths), not execution truth, and adding tables to the runtime store is
explicitly out of scope for a pure-addition increment.
...
| Field            | Type | Meaning |
|------------------|------|---------|
| `id`             | str  | Stable registry key (repo-style slug, e.g. `ai-command-center`). |
| `name`           | str  | Display name. |
| `kind`           | str  | One of `engine`, `control_plane`, `domain_module`, `governance`, `os`. |
| `project`        | str  | The `models.PROJECT_IDS` namespace its AICC tasks use (`AIOS`, `AICC`, `ESF`, `AML`, `AICOS`). Validated on write — this is the join key to the task board. |
| `repo_path`      | str \| null | Machine-local checkout path (`~` allowed). Null → health shows `unconfigured`, nothing else is collected. |
...
The file is seeded on first read with the five known projects (paths under
`~/Projects/…` / `~/Desktop/…` where they conventionally live); the seed is
config, not truth — the operator edits paths from the UI in a later
increment (increment 1 exposes registry state read-only; edits go through
`registry.upsert_entry` programmatically).
...
- **Per-project AIOS namespace federation** (multiple AIOS backends) — one
  backend, `project` field as namespace, as today.
