...
def test_first_read_seeds_the_five_known_projects(isolated_registry):
    ...
    # Every seeded namespace is a real task-board namespace — the join key
    # onto tasks_repository records.
...
def test_unknown_project_namespace_is_rejected(isolated_registry):
    with pytest.raises(registry.RegistryValidationError):
        registry.upsert_entry(
            {"id": "x", "kind": "engine", "project": "NOT_A_PROJECT"}
        )
...
def test_unknown_kind_and_unknown_fields_are_rejected(isolated_registry):
    ...
    with pytest.raises(registry.RegistryValidationError):
        registry.upsert_entry({"id": "x", "kind": "engine", "project": "AICC", "extra": 1})
