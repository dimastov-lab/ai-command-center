...
def _render_with_stubbed_health() -> None:
    ...
    tasks = [
        {"id": "t1", "project": "AIOS", "title": "Ship rc5", "status": "Backlog", "priority": "High"},
        {"id": "t2", "project": "AIOS", "title": "Done thing", "status": "Done", "priority": "Low"},
        {"id": "t3", "project": "AICC", "title": "Other project", "status": "Backlog", "priority": "Low"},
    ]
    runs = [
        {"id": "run12345", "project": "AIOS", "task_type": "implementation", "status": "completed", "created_at": "2026-08-12T09:00:00"},
    ...
    with (
        ...
        st.session_state["integration_center_health"] = {"aios": health}
        st.session_state["integration_drilldown"] = "aios"
...
def test_component_renders_health_badges_and_drilldown():
    ...
    assert "AIOS" in page_text
    # Drill-down: only the open AIOS task, not the Done one, not AICC's.
    assert "Ship rc5" in page_text
    assert "Done thing" not in page_text
    assert "Other project" not in page_text
    # Only the AIOS run appears in recent runs.
...
def test_component_shows_uncollected_state_without_calling_collectors():
    ...
    assert "AI Command Center" in captions
...
def test_full_app_page_renders_registry_without_network(monkeypatch):
    ...
    assert "ai-command-center" in page_text
