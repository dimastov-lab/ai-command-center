...
- Remote `Task.state` is authoritative for lanes and unsupported states or transitions fail explicitly. Outbound payloads exclude prompts and local paths; returned evidence preserves only event and request ID.
...
Complete W5-AICC-TASKS-GATEWAY-001 local verification, open a draft PR, and require exact-head CI plus human review. Do not begin dependent AICC cutover, merge, close PRs, deploy, rewrite history, or delete worktrees without explicit approval.
